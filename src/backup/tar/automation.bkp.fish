...
set src "/data/automation" # Variable qui contient le chemin vers le dossier à sauvegarder
set dst "/l/backup/automation" # Variable qui contient le chemin vers le dossier de destination de la sauvegarde
set arch "$dst/automation."(date +%Y%m%dT%H%M%S | tr -d :-)".tar.zst" # Variable qui contient le chemin vers l'archive de destination, avec un nom basé sur la date et l'heure
set log "/var/log/automation/automation.tar.bkp.log" # Variable qui contient le chemin vers le fichier de log
...
#i le dossier de destination n'existe pas, le créer
if test -d "$dst"
